...
function EmployeeMaster() {
  ...
  const handleFieldChange = (fieldName, value, setFormValues) => {
    ...
    if (lockedFieldsFromHiring.includes(fieldName)) {
      setFormValues((prev) => {
        ...
        if (prev.hiringId) {
          const selected = hiringOptions.find(
            (h) => h.value === Number(prev.hiringId)
          );
          ...
          if (selected) {
            ...
            if (!lockToastShown) {
              toast.warning(
                "This field comes from Hiring and cannot be changed."
              );
  ...
  const validate = (d) => {
    ...
    if (!/^\d{11}$/.test(String(d.contact || "")))
      return "Contact must be 11 digits.";
    if (!/^\d{13}$/.test(String(d.cnic || "")))
      return "CNIC must be 13 digits.";
    if (!d.email?.trim()) return "Email is required.";
    if (d.basicSalary == null || Number(d.basicSalary) < 0)
      return "Basic Salary must be 0 or more.";
    if (!d.status || !["Active", "Inactive"].includes(d.status))
  ...
  const handleSubmit = async (data) => {
    ...
    const payload = {
      ...trimmed,
      hiringID: trimmed.hiringId || initialValues.hiringID || null,
    };
  ...
  const handleEdit = (index) => {
    ...
    setInitialValues({
      ...
      joiningDate: fmt,
      status: e.status,
      hiringID: e.hiringId ?? e.hiringID ?? null, // ⭐ PRESERVE
    });
  ...
  const handleDelete = async (index) => {
    ...
    try {
      const res = await fetch(`http://localhost:5186/api/employees/${id}`, {
        method: "DELETE",
      });
  ...
  const handleViewHistory = async (index) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5186/api/employees/${emp.employeeID}/designation-history`
      );
  ...
  const rows = employees.map((e, idx) => ({
    ...e,
    // hiringID: e.hiringID,
    hiringID: e.hiringId ?? e.hiringID ?? "—",
        ...
        <div className="modal-overlay" onClick={closeHistory}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>
                Designation History – {historyEmployee?.employeeCode}{" "}
